...
from dataset_sar import DatasetFromHdf5
from ass import Loss_MRAE,SSIM,Loss_SAM,initialize_logger,AverageMeter,Loss_PSNR,Loss_RMSE,Loss_ERGAS
...
train_set = DatasetFromHdf5("data/PolSARFus_train.h5",True)
training_data_loader = DataLoader(dataset=train_set, num_workers=opt.threads, batch_size=opt.batchSize, shuffle=True)

test_set = DatasetFromHdf5("data/PolSARFus_test.h5",False)
test_data_loader = DataLoader(dataset=test_set, num_workers=opt.threads, batch_size=16, shuffle=False)
...
model =SpaXFus(mamban=4,hrms_channels=2,lrms_channels=9,mid=96,groups=3,ratio=ratio)
opt_chosed = 'adamax'

filepath = "checkpoint/PolSARFus/Mamba4ever_mid96_b" + str(opt.batchSize * opt.accumulation_steps) + "_" + opt_chosed + "_L1loss_lr" + str(opt.lr) + "/"
...
def train(training_data_loader,optimizer, model, criterion, epoch, accumulation_steps,scheduler,loss_vaule):
    ...
    for iteration, batch in enumerate(training_data_loader, 0):
        ...
        psnr=criterion_psnr(output[:, [0, 5, 8], :, :], target[:, [0, 5, 8], :, :])
        ssim = criterion_ssim(output[:, [0, 5, 8], :, :], target[:, [0, 5, 8], :, :])
        sam = criterion_sam(output[:, [0, 5, 8], :, :], target[:, [0, 5, 8], :, :])
...
def validate(val_loader, model,epoch):
    with torch.no_grad():
        ...
        for i,batch in enumerate(val_loader,0):
            ...
            if epoch%20==0:
                ...
                torch2png(mul[0, [0,5,8], :, :], imagepath+'Img'+str(i)+'PolSAR.png')
                torch2png(pan[0, 0, :, :].expand(3,-1,-1), imagepath+'Img'+str(i)+'SinSAR.png')
                torch2png(output[0, [0,5,8], :, :], imagepath+'Img'+str(i)+'output.png')
                torch2png(target[0, [0,5,8], :, :], imagepath + 'Img' + str(i) + 'gt.png')
                torch2tiff(mul[0, :, :, :], imagepath+'Tiff'+str(i)+'PolSAR.tiff')
                torch2tiff(pan[0, :, :, :], imagepath+'Tiff'+str(i)+'SinSAR.tiff')
                torch2tiff(output[0, :, :, :], imagepath+'Tiff'+str(i)+'output.tiff')
                torch2tiff(target[0, :, :, :], imagepath + 'Tiff' + str(i) + 'gt.tiff')
                writer.add_image('testimg/PolSAR'+str(i), mul[0, [0,5,8], :, :], epoch,dataformats='CHW')
                writer.add_image('testimg/SinSAR' + str(i), pan[0, 0, :, :].expand(3,-1,-1), epoch,dataformats='CHW')
                writer.add_image('testimg/Out' + str(i), output[0, [0,5,8], :, :], epoch,dataformats='CHW')
                writer.add_image('testimg/Targ' + str(i), target[0, [0,5,8], :, :], epoch,dataformats='CHW')

            q_output = output[:, [0, 5, 8], :, :]
            q_output = q_output / torch.max(q_output)
            q_target = target[:, [0, 5, 8], :, :]
            q_target = q_target / torch.max(q_target)

            loss_rmse = criterion_rmse(q_output, q_target)
            loss_psnr = criterion_psnr(q_output, q_target)
            loss_ssim = criterion_ssim(q_output, q_target)
            loss_sam = criterion_sam(q_output, q_target)
            loss_mrae = criterion_mrae(q_output, q_target)
            # record loss
    ...
    return losses_rmse.avg, losses_psnr.avg,losses_ssim.avg, losses_sam.avg, losses_mrae.avg
