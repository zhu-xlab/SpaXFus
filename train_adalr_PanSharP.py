...
train_set = DatasetFromHdf5("/home/jianghe/13MambaFus/data/GF2_train.h5",True)
training_data_loader = DataLoader(dataset=train_set, num_workers=opt.threads, batch_size=opt.batchSize, shuffle=True)

test_set = DatasetFromHdf5("/home/jianghe/13MambaFus/data/GF2_test.h5",False)
    ...
    with torch.no_grad():
        ...
        for i,batch in enumerate(val_loader,0):
            ...
            if epoch%20==0:
                ...
                torch2png(mul[0, [2, 1, 0], :, :], imagepath+'Img'+str(i)+'mul.png')
                torch2png(pan[0, :, :, :], imagepath+'Img'+str(i)+'pan.png')
                torch2png(output[0, [2, 1, 0], :, :], imagepath+'Img'+str(i)+'output.png')
                torch2png(target[0, [2, 1, 0], :, :], imagepath + 'Img' + str(i) + 'gt.png')
                torch2tiff(mul[0, :, :, :], imagepath+'Tiff'+str(i)+'mul.tiff')
                torch2tiff(pan[0, :, :, :], imagepath+'Tiff'+str(i)+'pan.tiff')
                torch2tiff(output[0, :, :, :], imagepath+'Tiff'+str(i)+'output.tiff')
                torch2tiff(target[0, :, :, :], imagepath + 'Tiff' + str(i) + 'gt.tiff')
                writer.add_image('testimg/Mul'+str(i), mul[0, [2, 1, 0], :, :], epoch,dataformats='CHW')
                writer.add_image('testimg/Pan' + str(i), pan[0, [2, 1, 0], :, :], epoch,dataformats='CHW')
