...
from dataset_stf import DatasetFromHdf5
...
train_set = DatasetFromHdf5("data/Land4Mod_train.h5",True)
training_data_loader = DataLoader(dataset=train_set, num_workers=opt.threads, batch_size=opt.batchSize, shuffle=True)

test_set = DatasetFromHdf5("data/Land4Mod_test.h5",False)
...
filepath = "checkpoint/Land4Mod/Mamba4ever_mid48_b" + str(opt.batchSize * opt.accumulation_steps) + "_" + opt_chosed + "_L1loss_lr" + str(opt.lr) + "/"
...
def train(training_data_loader,optimizer, model, criterion, epoch, accumulation_steps,scheduler,loss_vaule):
    ...
    for iteration, batch in enumerate(training_data_loader, 0):
        mod1, mod2, land1, land2 = Variable(batch[0]), Variable(batch[1]), Variable(batch[2]), Variable(batch[3])
        ...
        output = model(land1,torch.cat([mod1,mod2],1))
        loss = criterion(output, torch.cat([land1,target],1))
        output=output[:,4:,:,:]
    ...
    with torch.no_grad():
        ...
        for i,batch in enumerate(val_loader,0):
            ...
            if epoch%20==0:
                ...
                torch2png(mod1[0, [2, 1, 0], :, :], imagepath + 'Img' + str(i) + 'mod1.png')
                torch2png(mod2[0, [2, 1, 0], :, :], imagepath + 'Img' + str(i) + 'mod2.png')
                torch2png(output[0, [2, 1, 0], :, :], imagepath + 'Img' + str(i) + 'output.png')
                torch2png(target[0, [2, 1, 0], :, :], imagepath + 'Img' + str(i) + 'gt.png')
                torch2tiff(mod1[0, :, :, :], imagepath + 'Tiff' + str(i) + 'mod1.tiff')
                torch2tiff(mod2[0, :, :, :], imagepath + 'Tiff' + str(i) + 'mod2.tiff')
                torch2tiff(output[0, :, :, :], imagepath + 'Tiff' + str(i) + 'output.tiff')
                torch2tiff(target[0, :, :, :], imagepath + 'Tiff' + str(i) + 'gt.tiff')
                writer.add_image('testimg/mod1' + str(i), mod1[0, [2, 1, 0], :, :], epoch, dataformats='CHW')
                writer.add_image('testimg/mod2' + str(i), mod2[0, [2, 1, 0], :, :], epoch, dataformats='CHW')
                writer.add_image('testimg/Out' + str(i), output[0, [2, 1, 0], :, :], epoch, dataformats='CHW')
                writer.add_image('testimg/Targ' + str(i), target[0, [2, 1, 0], :, :], epoch, dataformats='CHW')
